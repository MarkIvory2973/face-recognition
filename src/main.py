...
def Train(datasets_root, checkpoints_root, total_epoch, learning_rate, batch_size, gamma):
    ...
    criterion = torch.nn.TripletMarginLoss(3.0, reduction="none")           # 三元损失
    ...
    for epoch in range(total_epoch):
        ...
        for step, (anchor, positive, negative) in enumerate(faces_train):
            ...
            loss_ = criterion(anchor_out, positive_out, negative_out)
            loss = loss_.mean()
            ...
            with torch.no_grad():
                ...
                accuracy_train += (loss_ == 0).sum().item()/(len(faces_train) * batch_size)     # 实时准确率
        ...
        with torch.no_grad():
            for step, (anchor, positive, negative) in enumerate(faces_test):
                ...
                accuracy_test += (loss_ == 0).sum().item()/(len(faces_test) * batch_size)   # 实时准确率
